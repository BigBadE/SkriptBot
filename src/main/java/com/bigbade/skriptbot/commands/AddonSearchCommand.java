...
public class AddonSearchCommand implements ICommand {
    @Getter
    private final List<String> aliases = Collections.unmodifiableList(Arrays.asList("a", "addon", "addons"));
    ...
    public void onCommand(Message message, String[] args) {
        if (args.length == 0) {
            MessageUtils.sendEmbedWithReaction(message.getTextChannel(),
                    MessageUtils.getErrorMessage(message.getId(), "No Addon Specified",
        ...
        if (!addon.isPresent()) {
            MessageUtils.sendEmbedWithReaction(message.getTextChannel(),
                    MessageUtils.getErrorMessage(message.getId(), "No Addon Found",
        ...
        if(dependencies.containsKey("softdepend")) {
            ...
            for (Object dependency : (JsonArray) ((JsonObject) data.get("depend")).get("softdepend")) {
                dependencyBuilder.append((String) dependency).append(", ");
            }
            softDependencies = dependencyBuilder.substring(0, dependencyBuilder.length()-2);
        } else {
            softDependencies = "None";
        }

        MessageUtils.sendEmbedWithReaction(message.getTextChannel(), new EmbedBuilder()
                .setTitle(data.getString(JsonKeys.PLUGIN.getKey()) + " " + data.getString(JsonKeys.VERSION.getKey()),
                        data.getString(JsonKeys.DOWNLOAD.getKey()))
                .setColor(Color.GREEN)
                .addField("Addon", "**" + addon.get() + "** by **" + authorBuilder + "**", true)
                .addField("Description", data.getString(JsonKeys.DESCRIPTION.getKey()), true)
                .addField("Soft Depends", softDependencies, true)
                .addField("Download (" + shortByteCount(data.getLong(JsonKeys.BYTES.getKey())) + ")",
                        data.getString(JsonKeys.DOWNLOAD.getKey()), false)
                .setFooter("Data provided by Duetro's Skript API").build());
    }
     ...
     */
    public static String shortByteCount(long bytes) {
