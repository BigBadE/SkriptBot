...
public class DocSearchCommand implements ICommand {
  ...
  private final String[] aliases =
      new String[] {"d", "doc", "docs", "documentation"};
  ...
  public static MessageEmbed getResponse(JsonArray array, String id,
                                         int index) {
    JsonObject object = (JsonObject)array.get(index);
    EmbedBuilder builder =
        new EmbedBuilder()
            .setTitle(
                object.getString(JsonKeys.NAME.getKey()),
            ...
            .addField("Pattern",
                      "```" + object.getString(JsonKeys.PATTERN.getKey()) +
                          "```",
                      false);
    String description = object.getString(JsonKeys.DESC.getKey());
    if (!description.isEmpty()) {
      builder.addField("Description", object.getString(JsonKeys.DESC.getKey()),
                       false);
    }
    JsonArray examples = (JsonArray)object.get("examples");
    if (!examples.isEmpty()) {
      builder.addField("Example",
                       "```" +
                           HTMLUtilities.unescapeHtml(
                               ((JsonObject)examples.get(0))
                                   .getString(JsonKeys.EXAMPLE.getKey())) +
                           "```",
                       false);
    }
  ...
  public void onCommand(Member sender, TextChannel channel, String id,
    ...
    if (args.length == 0) {
      MessageUtils.sendEmbedWithReaction(
          channel,
          MessageUtils.getErrorMessage(id, "No Syntax Specified",
    ...
    if (array.isEmpty()) {
      MessageUtils.sendEmbedWithReaction(
          channel,
          MessageUtils.getErrorMessage(id, "No Results",
    ...
    channel.sendMessage(builder.build())
        .queue(response -> queNextAction(response, new AtomicInteger(0)));
  }
  ...
  private static void queNextAction(Message message, AtomicInteger current) {
    ...
    message.addReaction(reaction).queue(
        none -> queNextAction(message, current), throwable -> {});
  }

  public void addDocsResponse(EmbedBuilder builder, JsonArray array,
                              int start) {
    for (int i = start; i < Math.min(array.size(), start + 10); i++) {
      JsonObject result = (JsonObject)array.get(i);
      String doc = result.getString(JsonKeys.DOC.getKey());
      builder.addField(
          i + ". " + result.getString(JsonKeys.NAME.getKey()) + " (" +
  ...
  @Override
  public void onReaction(User reactor, Message command, Message message,
    ...
    String query =
        RegexPatterns.SPACE_PATTERN.split(command.getContentDisplay(), 2)[1];
    int found = getNumberFromString(Objects.requireNonNull(
        message.getEmbeds().get(0).getFields().get(0).getName()));
    JsonArray array = dataFetcher.getDocsResults(query);
  ...
  private void scrollCommand(Message message, JsonArray array, int index) {
    EmbedBuilder builder =
        new EmbedBuilder().setFooter(FOOTER).setColor(Color.GREEN);
