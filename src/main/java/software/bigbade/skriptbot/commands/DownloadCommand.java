...
public class DownloadCommand implements ICommand {
  ...
  private final String[] aliases =
      new String[] {"down", "download", "downlaods"};

  public static final EmbedBuilder EMBED =
      new EmbedBuilder()
          .setTitle("Downloading Skript")
          .setColor(Color.GREEN)
          .addField(
              "1.7",
              "Recommended but **not supported**:\n\n"
                  +
                  "[Njol, 2.2-SNAPSHOT](https://github.com/Pikachu920/Skript/releases/download/2.2-SNAPSHOT/Skript-2.2-Njol.jar)",
              true)
          .addField(
              "1.8",
              "Recommended but **not supported**:\n\n"
                  +
                  "[Bensku fork, dev36](https://github.com/SkriptLang/Skript/releases/download/dev36/Skript.jar)",
              true)
          .addField(
              "1.9+",
              "Recommended: \n\n"
          ...
          .addField(
              "Extra Info",
              "Note that all versions of Skript not by Njol are unofficial and "
                  +
                  "their issues should be reported to their respective issue trackers",
              false)
          .setFooter("Downloads | ");
  ...
  public void onCommand(Member sender, TextChannel channel, String id,
    ...
    MessageUtils.sendEmbedWithReaction(
        channel,
        new EmbedBuilder(EMBED).setFooter("Downloads | " + id).build());
  }
}
