...
public class FormattingCommand implements ICommand {
  public static final EmbedBuilder FORMATTING_EMBED =
      new EmbedBuilder()
          .setTitle("Formatting Code In Discord")
          ...
          .addField(new MessageEmbed.Field(
              "Extra Info",
              "On US keyboards, the grave character (`) is "
                  + "located above the tab key on the top left of the keyboard",
              false));
  @Getter
  ...
  public void onCommand(Member sender, TextChannel channel, String id,
    ...
    MessageUtils.sendEmbedWithReaction(
        channel,
        new EmbedBuilder(FORMATTING_EMBED).setFooter("Format | " + id).build());
  }
}
