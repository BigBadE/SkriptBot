...
public class PastebinCommand implements ICommand {
  @Getter private final String[] aliases = new String[] {"pastebin", "paste"};
  ...
  public void onCommand(Member sender, TextChannel channel, String id,
    ...
    channel.retrieveMessageById(args[0])
        ...
        .queue(message -> {
          ...
          if (message.getAttachments().isEmpty()) {
            MessageUtils.sendEmbedWithReaction(
                channel, MessageUtils.getErrorMessage(
                             id, "No File Found",
                             "There was no attached file to that message."));
            return;
          }
          message.getAttachments().get(0).retrieveInputStream().thenAccept(
              inputStream -> {
  ...
  private void sendPastebin(TextChannel channel, Message message, String id,
                            String input) {
    Map<String, String> arguments = new HashMap<>();
    arguments.put("api_option", "paste");
    arguments.put("api_paste_private", "1");
    Message.Attachment attachment = message.getAttachments().get(0);
    arguments.put("api_paste_name", message.getAuthor().getName() +
                                        "'s File: " + attachment.getFileName());
    ...
    Optional<InputStream> response = HTTPUtilities.sendPostRequest(
        "https://pastebin.com/api/api_post.php", arguments);
    if (!response.isPresent()) {
      MessageUtils.sendEmbedWithReaction(
          channel, MessageUtils.getErrorMessage(id, "Error Creating Paste",
